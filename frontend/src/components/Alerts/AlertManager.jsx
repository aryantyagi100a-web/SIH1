...
const INPUT_CLS = 'w-full bg-neutral-900 border border-neutral-700 rounded-xl px-3 py-2.5 text-white text-fluid-xs focus:outline-none focus:border-white min-h-[44px] transition-colors';
...
export default function AlertManager() {
  ...
  /* Restrict Unauthorized Mass SMS Broadcasting */
  if (!isAuthenticated) {
    return (
        ...
        <div className="glass-panel p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-neutral-800 shadow-[0_20px_50px_rgba(0,0,0,0.8)] space-y-5 sm:space-y-6">
          <div className="text-center space-y-2 sm:space-y-3">
            ...
            <div className="space-y-1">
              ...
              <h1 className="text-fluid-xl font-bold tracking-tight text-white mt-2">Emergency Alert & SMS Dispatcher</h1>
              <p className="text-fluid-xs text-neutral-400 max-w-md mx-auto leading-relaxed">Restricted to verified Disaster Management Officers (NDMA, SDMA, NDRF, & District Magistrates).</p>
            </div>
          </div>

          <div className="bg-neutral-950 p-3.5 sm:p-4 rounded-2xl border border-neutral-800 space-y-2 text-fluid-xs">
            <div className="flex items-center space-x-2 text-emerald-400 font-bold text-fluid-xs">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-neutral-300 text-fluid-xs font-semibold mb-1">Officer Name / Badge ID:</label>
            ...
            <div>
              <label className="block text-neutral-300 text-fluid-xs font-semibold mb-1">Officer Password:</label>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3.5 text-neutral-500 hover:text-white transition-colors touch-target">
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 pt-2">
              <button type="submit" className="flex-1 py-3 min-h-[44px] bg-white text-black hover:bg-neutral-200 font-bold text-fluid-xs rounded-xl flex items-center justify-center space-x-2 transition-all shadow-lg active:scale-98">
                <Lock className="w-4 h-4" />
                <span>Sign In & Unlock Dispatcher</span>
              </button>
              <button type="button" onClick={handleQuickDemoLogin} className="py-3 min-h-[44px] px-4 bg-neutral-900 hover:bg-neutral-800 text-neutral-300 border border-neutral-700 font-semibold text-fluid-xs rounded-xl flex items-center justify-center space-x-1.5 transition-colors active:scale-98">
    ...
    <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6 animate-fade-in py-1 sm:py-2">
      {toastMessage && (
        <div className={`p-3.5 sm:p-4 rounded-xl border flex items-center space-x-3 transition-all ${toastMessage.type === 'success' ? 'bg-neutral-900 border-emerald-500/40 text-emerald-300' : 'bg-neutral-900 border-red-500/40 text-red-300'}`}>
          ...
          <span className="text-fluid-xs font-mono">{toastMessage.text}</span>
        </div>
      )}

      {/* Authenticated Officer Status Header */}
      <div className="glass-panel p-4 rounded-2xl border border-neutral-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-neutral-950/80">
        <div className="flex items-center space-x-3">
          ...
          <div className="truncate">
            <div className="flex items-center space-x-2">
              <span className="text-fluid-xs font-bold text-white truncate">OFFICIAL DISPATCH ACTIVE</span>
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <button onClick={() => setShowCreateModal(true)} className="flex-1 sm:flex-initial px-4 py-2.5 min-h-[44px] bg-white text-black hover:bg-neutral-200 text-fluid-xs font-bold rounded-xl shadow-lg flex items-center justify-center space-x-1.5 transition-all active:scale-98">
            <Radio className="w-4 h-4 text-black animate-pulse flex-shrink-0" />
            <span className="truncate">Broadcast Alert</span>
          </button>
          <button onClick={handleLogout} className="px-3.5 py-2.5 min-h-[44px] bg-neutral-900 hover:bg-neutral-800 text-neutral-400 hover:text-white border border-neutral-800 text-fluid-xs rounded-xl transition-colors flex items-center space-x-1.5 flex-shrink-0">
            <Lock className="w-4 h-4" />
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5 text-fluid-xs pt-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-neutral-400 font-mono text-[10px] sm:text-[11px] uppercase">Filter:</span>
          <select value={selectedState} onChange={(e) => setSelectedState(e.target.value)} className="bg-neutral-900 text-neutral-200 rounded-xl border border-neutral-800 px-3 py-2 text-fluid-xs min-h-[40px] focus:outline-none">
            {STATES.map(s => <option key={s} value={s}>{s === 'All' ? t.allStates : s}</option>)}
          </select>
          <select value={selectedSeverity} onChange={(e) => setSelectedSeverity(e.target.value)} className="bg-neutral-900 text-neutral-200 rounded-xl border border-neutral-800 px-3 py-2 text-fluid-xs min-h-[40px] focus:outline-none">
        ...
        {filtered.map((alert) => {
          ...
          return (
            <div key={alert.id} className="glass-panel p-4 sm:p-5 rounded-2xl border border-neutral-800 hover:border-neutral-700 transition-all space-y-3 bg-black">
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-1 truncate pr-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-fluid-xs text-neutral-400 font-medium">{alert.state} • {alert.district}</span>
                  </div>
                  <h3 className="text-fluid-base font-bold text-white tracking-tight leading-snug">{alert.title}</h3>
                </div>
                <span className={`px-2.5 py-1 rounded-lg text-[9px] sm:text-[10px] font-mono font-bold tracking-wider flex-shrink-0 ${
              ...
              <p className="text-fluid-xs text-neutral-300 leading-relaxed bg-neutral-950 p-3 rounded-xl border border-neutral-800 font-mono">{alert.message}</p>
              <div className="pt-2 border-t border-neutral-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-fluid-xs">
                ...
                <button onClick={() => handleBroadcastSMS(alert.id)} disabled={broadcastingId === alert.id} className="w-full sm:w-auto px-4 py-2.5 min-h-[44px] bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-700 text-fluid-xs font-semibold rounded-xl flex items-center justify-center space-x-2 transition-colors active:scale-98">
      ...
      {/* Emergency Creation Modal Dialog */}
      {showCreateModal && (
          ...
          <div className="glass-panel p-5 sm:p-6 rounded-2xl sm:rounded-3xl max-w-lg w-full border border-neutral-800 shadow-2xl space-y-4 bg-black max-h-[90vh] overflow-y-auto my-auto animate-fade-in">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-fluid-base font-bold text-white">Emergency Citizen SMS Dispatch</h3>
              </div>
              <button onClick={() => setShowCreateModal(false)} className="p-2 rounded-lg text-neutral-400 hover:text-white touch-target flex items-center justify-center">✕</button>
            </div>
            <form onSubmit={handleCreateAlert} className="space-y-3.5 text-fluid-xs">
              <div>
              ...
              <div>
                ...
                <textarea rows={3} required placeholder="e.g. SDMA URGENT: High landslide probability on North Sikkim Highway." value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} className={`${INPUT_CLS} leading-relaxed min-h-[70px]`} />
              </div>
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-2.5 pt-2">
                <button type="button" onClick={() => setShowCreateModal(false)} className="w-full sm:w-auto px-4 py-2.5 min-h-[44px] text-neutral-400 hover:text-white text-fluid-xs">{t.cancel}</button>
                <button type="submit" className="w-full sm:w-auto px-5 py-3 min-h-[44px] bg-white text-black font-bold rounded-xl shadow-lg hover:bg-neutral-200 text-fluid-xs flex items-center justify-center space-x-2 active:scale-98">
