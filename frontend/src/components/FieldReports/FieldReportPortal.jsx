...
const INP_CLS = 'w-full bg-[#070b14] border border-white/[0.08] rounded-lg px-3 py-2 text-white focus:outline-none text-fluid-xs min-h-[44px] transition-colors focus:border-white/30';
...
export default function FieldReportPortal() {
  ...
  return (
    /* 
      Crowdsourced Field Intel Container:
      Uses a 12-column grid layout (5-span form, 7-span live feed) that stacks cleanly on phones (< 1024px).
    */
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 animate-fade-in py-1 sm:py-2">
      <div className="pb-4 border-b border-white/[0.06]">
        ...
        <h1 className="text-fluid-xl font-bold tracking-tight text-[#e2e8f0] mt-1">{t.reportFormTitle}</h1>
        <p className="text-fluid-xs text-[#64748b] mt-1">{t.reportFormSubtitle}</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
        {/* Left Column: Mobile-Friendly Field Submission Form */}
        <div className="lg:col-span-5 glass-panel p-4 sm:p-6 rounded-2xl border border-white/[0.06] space-y-4">
          {submittedSuccess && (
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-lg text-fluid-xs font-mono flex items-center space-x-2">
              <CheckCircle className="w-4 h-4 flex-shrink-0" />
              <span>Report transmitted to Disaster Command Center!</span>
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-3.5 text-fluid-xs">
            ...
            {/* GPS Location Pin Input Box with 44px touch targets */}
            <div className="p-3.5 bg-[#070b14] rounded-xl border border-white/[0.06] space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[10px] sm:text-[11px] text-[#64748b] font-mono uppercase tracking-wider">GPS PIN COORDINATES</span>
                <button type="button" onClick={handleGetLocation} className="px-3 py-2 min-h-[44px] bg-white/[0.08] hover:bg-white/[0.15] text-white rounded-lg text-fluid-xs font-mono flex items-center space-x-1.5 active:scale-95 transition-all">
                  <Navigation className="w-4 h-4 text-emerald-400" />
                  <span>{geoLocating ? t.acquiringGps : t.autoGps}</span>
                </button>
              </div>
              <div className="grid grid-cols-2 gap-2 text-fluid-xs font-mono">
                <input type="text" value={formData.lat} onChange={(e) => setFormData({ ...formData, lat: e.target.value })} className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg p-2 min-h-[44px] text-white" placeholder="Latitude" />
                <input type="text" value={formData.lng} onChange={(e) => setFormData({ ...formData, lng: e.target.value })} className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg p-2 min-h-[44px] text-white" placeholder="Longitude" />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea rows={2} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full bg-[#070b14] border border-white/[0.08] rounded-lg p-2.5 text-white focus:outline-none text-fluid-xs min-h-[60px]" />
            </div>

            <button type="submit" className="w-full py-3 min-h-[44px] bg-white text-black hover:bg-neutral-200 font-bold text-fluid-xs rounded-xl shadow-lg flex items-center justify-center space-x-2 transition-all active:scale-98">
        ...
        {/* Right Column: Verified Live Ground Reports List */}
              ...
              <div key={rep.id} className="glass-panel p-4 rounded-xl border border-white/[0.06] hover:border-white/[0.12] transition-all space-y-2.5">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h4 className="text-fluid-base font-semibold text-white mt-0.5">{rep.hazardType}</h4>
                  </div>
                ...
                <p className="text-fluid-xs text-[#94a3b8] leading-relaxed bg-[#070b14]/50 p-2.5 rounded-lg border border-white/[0.04]">"{rep.description}"</p>
                <div className="flex items-center justify-between text-fluid-xs text-[#64748b] pt-1">
                  <span>{rep.reporterName} ({rep.role})</span>
                  {!rep.verified && (
                    <button onClick={() => handleVerify(rep.id)} className="px-3 py-2 min-h-[44px] bg-white/[0.06] hover:bg-white/[0.1] text-white rounded-lg text-fluid-xs font-mono transition-colors flex items-center">
                      {t.verifyAction}
                    </button>
                  )}
                </div>
