    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12 animate-fade-in">
      <div className="border-b border-neutral-800 pb-5">
        <div className="flex items-center space-x-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white">{t.reliefTitle || 'Disaster Relief Fund & Community Aid'}</h1>
            <p className="text-xs text-neutral-400 mt-0.5">{t.reliefSubtitle || 'Direct citizen aid for families affected by North East landslides. 100% verified.'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          {[
            ...
            <div key={i} className="bg-black border border-neutral-800 p-3 rounded-xl">
              <span className="text-[10px] font-mono text-neutral-400 uppercase block">{m.label}</span>
              <span className="text-lg font-mono font-bold text-white tracking-tight">{m.val}</span>
              <span className={`text-[10px] ${m.col} block mt-0.5 font-mono`}>{m.sub}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-black border border-neutral-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-base font-bold text-white flex items-center gap-2"><DollarSign className="w-4 h-4 text-emerald-400" /> {t.donateNow || 'Quick Direct Contribution'}</h2>
                <p className="text-xs text-neutral-400 mt-0.5">{t.selectAmount || 'Select amount to contribute via UPI'}</p>
              </div>
              <span className="text-[10px] font-mono uppercase bg-neutral-900 border border-neutral-800 px-2.5 py-1 rounded-full text-neutral-400">0% Fee</span>
            </div>

            <div className="grid grid-cols-4 gap-2 mb-4">
              {[500, 1000, 2500, 5000].map((amt) => (
                <button key={amt} type="button" onClick={() => { setSelectedAmount(amt); setCustomAmount(''); }} className={`py-3 rounded-xl text-xs font-mono font-bold border transition-all ${selectedAmount === amt && !customAmount ? 'bg-white text-black border-white shadow-[0_0_15px_rgba(255,255,255,0.2)]' : 'bg-neutral-900/80 text-neutral-300 border-neutral-800 hover:text-white'}`}>
                  ₹{amt.toLocaleString('en-IN')}
                </button>
              ))}
            </div>

            <input type="number" placeholder={t.customAmount || 'Enter custom amount (₹)'} value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setSelectedAmount(null); }} className={`${INP_CLS} py-2.5 mb-5`} />

            <div className="bg-neutral-900/40 border border-neutral-800/80 rounded-xl p-4 flex flex-col sm:flex-row items-center gap-4">
              <div className="bg-white p-2.5 rounded-lg flex-shrink-0">
                <div className="w-24 h-24 bg-neutral-900 rounded flex flex-col items-center justify-center p-1 text-center">
              ...
              <div className="space-y-2 flex-1 text-center sm:text-left">
                ...
                <div className="flex items-center gap-2 bg-black border border-neutral-800 px-3 py-1.5 rounded-lg">
                  <span className="text-[11px] font-mono text-neutral-300 truncate">ner.disaster.relief@gov.in</span>
                ...
                <p className="text-[10px] text-neutral-400">Direct transfer to Central NER Disaster Management Cell.</p>
              </div>
            </div>

            <button type="button" onClick={handleDonate} className="w-full mt-4 bg-white hover:bg-neutral-200 text-black py-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.15)]">
          ...
          <div className="bg-black border border-neutral-800 rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
              <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2"><Building2 className="w-3.5 h-3.5 text-neutral-400" /> Official State Relief Bank Accounts</h3>
              <span className="text-[10px] font-mono text-emerald-400 flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5" /> 80G Tax Exempt</span>
            </div>
              ...
              {OFFICIAL_ACCOUNTS.map((acc, idx) => (
                <div key={idx} className="bg-neutral-900/60 border border-neutral-800/80 rounded-xl p-3.5 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-mono text-neutral-400 bg-neutral-950 border border-neutral-800 px-2 py-0.5 rounded">{acc.bank}</span>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px] font-mono">
                    ...
                    <div className="bg-black/60 p-2 rounded border border-neutral-800/60 col-span-2 sm:col-span-1">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-black border border-neutral-800 rounded-2xl p-5 space-y-4">
            ...
            {pledgeSubmitted ? (
              ...
              <form onSubmit={handlePledge} className="space-y-3">
                ...
                <button type="submit" className="w-full bg-neutral-900 hover:bg-neutral-800 border border-neutral-700 text-white py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 mt-2">
          ...
          <div className="bg-black border border-neutral-800 rounded-2xl p-5 space-y-3">
              ...
              {donations.slice(0, 5).map((d) => (
                <div key={d.id} className="flex items-center justify-between p-2 rounded-lg bg-neutral-900/40 border border-neutral-800/60 text-xs">
                  <div>
                    ...
                    <span className="text-[10px] text-neutral-400 font-mono">{d.target} &bull; {d.time}</span>
                  </div>
