...
const INP_CLS = 'w-full bg-neutral-900 border border-neutral-800 rounded-xl px-3 py-2.5 text-fluid-xs text-white focus:outline-none focus:border-white min-h-[44px] transition-colors';
...
export default function ReliefFundPortal() {
  ...
  return (
    /* 
      Relief & Aid Portal:
      Structured layout that adapts seamlessly across phone screens (320px-480px) and wide desktop monitors.
    */
    <div className="space-y-5 sm:space-y-6 max-w-7xl mx-auto pb-6 sm:pb-12 animate-fade-in py-1">
      <div className="border-b border-neutral-800 pb-4 sm:pb-5">
        <div className="flex items-center space-x-3 mb-2">
          <div className="p-2.5 bg-neutral-900 border border-neutral-800 rounded-xl flex-shrink-0">
            <Heart className="w-6 h-6 text-red-500 fill-red-500/20" />
          </div>
          <div>
            <h1 className="text-fluid-xl font-bold tracking-tight text-white">{t.reliefTitle || 'Disaster Relief Fund & Community Aid'}</h1>
            <p className="text-fluid-xs text-neutral-400 mt-0.5">{t.reliefSubtitle || 'Direct citizen aid for families affected by North East landslides. 100% verified.'}</p>
          </div>
        </div>

        {/* Dynamic Metric Cards: 2 cols on phone, 4 cols on desktop */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 mt-3 sm:mt-4">
          {[
            ...
            <div key={i} className="bg-black border border-neutral-800 p-3 rounded-xl">
              <span className="text-[9px] sm:text-[10px] font-mono text-neutral-400 uppercase block truncate">{m.label}</span>
              <span className="text-fluid-base font-mono font-bold text-white tracking-tight block truncate">{m.val}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
        {/* Left Column: Direct UPI & Bank Transfer */}
        <div className="lg:col-span-7 space-y-5 sm:space-y-6">
          <div className="bg-black border border-neutral-800 rounded-2xl p-4 sm:p-6">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <div>
                <h2 className="text-fluid-base font-bold text-white flex items-center gap-2"><DollarSign className="w-4 h-4 text-emerald-400" /> {t.donateNow || 'Quick Direct Contribution'}</h2>
                <p className="text-fluid-xs text-neutral-400 mt-0.5">{t.selectAmount || 'Select amount to contribute via UPI'}</p>
              </div>
            ...
            {/* Touch-Friendly Quick Amount Selector (Min 44px hit target) */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3 sm:mb-4">
              {[500, 1000, 2500, 5000].map((amt) => (
                <button key={amt} type="button" onClick={() => { setSelectedAmount(amt); setCustomAmount(''); }} className={`py-3 min-h-[44px] rounded-xl text-fluid-xs font-mono font-bold border transition-all active:scale-95 flex items-center justify-center ${selectedAmount === amt && !customAmount ? 'bg-white text-black border-white shadow-[0_0_15px_rgba(255,255,255,0.2)]' : 'bg-neutral-900/80 text-neutral-300 border-neutral-800 hover:text-white'}`}>
                  ₹{amt.toLocaleString('en-IN')}
                </button>
              ))}
            </div>

            <input type="number" placeholder={t.customAmount || 'Enter custom amount (₹)'} value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setSelectedAmount(null); }} className={`${INP_CLS} mb-4`} />

            {/* UPI QR Code Container (Aspect Ratio Preserved) */}
            <div className="bg-neutral-900/40 border border-neutral-800/80 rounded-xl p-3.5 sm:p-4 flex flex-col sm:flex-row items-center gap-3.5 sm:gap-4">
              <div className="bg-white p-2.5 rounded-xl flex-shrink-0 aspect-square">
                <div className="w-20 h-20 sm:w-24 sm:h-24 bg-neutral-900 rounded-lg flex flex-col items-center justify-center p-1 text-center">
              ...
              <div className="space-y-1.5 sm:space-y-2 flex-1 text-center sm:text-left w-full truncate">
                <span className="text-fluid-xs font-bold text-white block">{t.scanQr || 'Scan with GPay, PhonePe, Paytm, or BHIM'}</span>
                <div className="flex items-center gap-2 bg-black border border-neutral-800 px-3 py-2 rounded-lg">
                  <span className="text-fluid-xs font-mono text-neutral-300 truncate">ner.disaster.relief@gov.in</span>
                  <button type="button" onClick={() => handleCopy(setCopiedUpi, 'ner.disaster.relief@gov.in')} className="ml-auto text-neutral-400 hover:text-white p-1 touch-target">
            ...
            <button type="button" onClick={handleDonate} className="w-full mt-3 sm:mt-4 bg-white hover:bg-neutral-200 text-black py-3.5 min-h-[44px] rounded-xl text-fluid-xs font-bold transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.15)] active:scale-98">
          ...
          <div className="bg-black border border-neutral-800 rounded-2xl p-4 sm:p-5 space-y-3 sm:space-y-4">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
              <h3 className="text-fluid-xs font-bold text-white uppercase tracking-wider flex items-center gap-2 truncate"><Building2 className="w-4 h-4 text-neutral-400 flex-shrink-0" /> State Bank Accounts</h3>
              <span className="text-[9px] sm:text-[10px] font-mono text-emerald-400 flex items-center gap-1 flex-shrink-0"><ShieldCheck className="w-4 h-4" /> 80G Exempt</span>
            </div>
            <div className="space-y-3">
              {OFFICIAL_ACCOUNTS.map((acc, idx) => (
                <div key={idx} className="bg-neutral-900/60 border border-neutral-800/80 rounded-xl p-3.5 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-fluid-xs font-bold text-white">{acc.state}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-fluid-xs font-mono">
                    <div className="bg-black/60 p-2 rounded-lg border border-neutral-800/60">
                      ...
                      <div className="flex items-center justify-between text-neutral-200">
                        ...
                        <button onClick={() => handleCopy(setCopiedAcc, acc.accountNo, idx)} className="text-neutral-400 hover:text-white p-1">
                          {copiedAcc === idx ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                        </button>
                      </div>
                    </div>
                    <div className="bg-black/60 p-2 rounded-lg border border-neutral-800/60">
        ...
        {/* Right Column: Physical Supply Pledge Form */}
        <div className="lg:col-span-5 space-y-5 sm:space-y-6">
          <div className="bg-black border border-neutral-800 rounded-2xl p-4 sm:p-5 space-y-3 sm:space-y-4">
            <h3 className="text-fluid-xs font-bold text-white uppercase tracking-wider flex items-center gap-2"><Package className="w-4 h-4 text-amber-400" /> {t.pledgeSupplies || 'Pledge Physical Relief Supplies'}</h3>
            {pledgeSubmitted ? (
              <div className="bg-emerald-950/40 border border-emerald-500/40 rounded-xl p-4 text-center space-y-2">
                <CheckCircle2 className="w-6 h-6 text-emerald-400 mx-auto" />
                <span className="text-fluid-xs font-bold text-white block">Pledge Registered Successfully!</span>
                <p className="text-fluid-xs text-emerald-300">Our regional nodal officer will SMS you nearest drop-off depot.</p>
              </div>
            ) : (
              <form onSubmit={handlePledge} className="space-y-3">
                ...
                <button type="submit" className="w-full bg-neutral-900 hover:bg-neutral-800 border border-neutral-700 text-white py-3 min-h-[44px] rounded-xl text-fluid-xs font-bold transition-all flex items-center justify-center gap-2 mt-2 active:scale-98">
          ...
          <div className="bg-black border border-neutral-800 rounded-2xl p-4 sm:p-5 space-y-3">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-2.5">
              <h3 className="text-fluid-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5"><Users className="w-4 h-4 text-neutral-400" /> Recent Community Donations</h3>
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            </div>
            <div className="space-y-2">
              {donations.slice(0, 5).map((d) => (
                <div key={d.id} className="flex items-center justify-between p-2.5 rounded-lg bg-neutral-900/40 border border-neutral-800/60 text-fluid-xs">
